...
function obtenerUsuarios()
{
    ...
    // Compatibilidad con la primera versión de la persistencia de la EP1.
    try
    {
        const usuarioAnterior = JSON.parse(localStorage.getItem("nutrivida_usuario_demo"));
        if (usuarioAnterior)
        {
            const migrado = [
            {
                ...usuarioAnterior, id: 3, rol: "Paciente", estado: "Activo"
            }
            ];
            localStorage.setItem(USUARIOS_KEY, JSON.stringify(migrado));
            return migrado;
        }
    }
    catch (error)
    {
        console.warn("No se pudo migrar el usuario de demostración.", error);
    }
    return [];
}
...
document.addEventListener("DOMContentLoaded", () =>
{
    ...
    region.addEventListener("change", () =>
    {
        ...
        (comunasPorRegion[region.value] || []).forEach((nombreComuna) =>
        {
            const option = document.createElement("option");
    ...
    form.addEventListener("submit", (event) =>
    {
        ...
        const runNormalizado = values.run.trim().toUpperCase();
        const correoNormalizado = values.correo.trim().toLowerCase();
        if (!validarRun(runNormalizado)) return setFieldError("run", "Ingresa un RUN chileno válido.");
        ...
        if (!validEmail(correoNormalizado)) return setFieldError("correo", "Usa @duoc.cl, @profesor.duoc.cl, @gmail.com o @nutrivida.cl.");
        if (values["fecha-nacimiento"] && new Date(values["fecha-nacimiento"] + "T00:00:00") > new Date())
        {
            return setFieldError("fecha-nacimiento", "La fecha no puede ser futura.");
        }
        ...
        if (values.password.length < 6 || values.password.length > 20) return setFieldError("password", "La contraseña debe tener entre 6 y 20 caracteres.");
        if (values.password !== values["password-confirmar"]) return setFieldError("password-confirmar", "Las contraseñas no coinciden.");
        const usuarios = obtenerUsuarios();
        const duplicadoRun = usuarios.some((usuario) => usuario.run === runNormalizado);
        const duplicadoCorreo = usuarios.some((usuario) => usuario.correo === correoNormalizado);
        if (duplicadoRun) return setFieldError("run", "Este RUN ya está registrado.");
        if (duplicadoCorreo) return setFieldError("correo", "Este correo ya está registrado.");
        const siguienteId = usuarios.reduce((max, usuario) => Math.max(max, Number(usuario.id) || 0), 2) + 1;
        // No se almacena la contraseña: solo se persisten los datos necesarios para la demostración de la EP1.
        const usuario =
        {
            id: siguienteId,             run: runNormalizado,             nombre: values.nombre.trim(),             apellidos: values.apellidos.trim(),             correo: correoNormalizado,             fechaNacimiento: values["fecha-nacimiento"] || "",             region: values.region,             comuna: values.comuna,             direccion: values.direccion.trim(),             rol: "Paciente",             estado: "Activo"
        }
        ;
        usuarios.push(usuario);
        guardarUsuarios(usuarios);
        showFormMessage("mensaje-registro", "Registro realizado correctamente. El usuario quedó disponible en Administración → Usuarios.", "success");
        form.reset();
        comuna.innerHTML = '<option value="">Selecciona una comuna</option>';
